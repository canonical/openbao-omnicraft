...
This repository contains the source code for the `openbao-k8s` and `openbao` charms. You can find the contributing guidelines in each charm's directory if you want to contribute to one of these charms.
...
Shared code used by both the OpenBao machine and K8s charms is defined in `openbao-package`. This code is vendored into both charms so that `charmcraft pack` can run in each directory without any additional information. Vendored code **must** be in sync to merge to `main`.
