"""Library for managing OpenBao Charm features.

This library encapsulates the business logic for managing the OpenBao service and
its associated integrations within the context of our charms.

A OpenBao Feature Manager will aim to encapsulate as much of the business logic
...
enhance the offering of the Charm by interacting with the OpenBao Service to
...
from openbao.openbao_s3 import S3, S3Error
...
CA_CERTIFICATE_JUJU_SECRET_LABEL = "self-signed-openbao-ca-certificate"

OPENBAO_CA_SUBJECT = "OpenBao self signed CA"
...
_OPENBAO_PROVIDER_NAME = "openbao-pki"
_CERTIFICATE_ERROR_MESSAGES = {
    CertificateRequestErrorCode.IP_NOT_ALLOWED: (
        "OpenBao PKI role rejected the IP Subject Alternative Name."
    ),
    CertificateRequestErrorCode.DOMAIN_NOT_ALLOWED: (
        "OpenBao PKI role rejected the requested DNS names."
    ),
    CertificateRequestErrorCode.OTHER: ("OpenBao PKI role rejected the certificate request."),
}


def _map_openbao_pki_errors(
    error: PKICertificateError,
) -> CertificateError:
    """Map OpenBao PKI errors to canonical certificate errors from the tls library.

    Args:
        error: PKICertificateError, The error object wrapping different OpenBao PKI errors.
    ...
    return CertificateError(
        ...
        reason=str(error),
        provider=_OPENBAO_PROVIDER_NAME,
    )
...
class OpenBaoCertsError(Exception):
    """Exception raised when a openbao certificate is not found."""

    def __init__(
        self, message: str = "Could not retrieve openbao certificates from local storage"
    ):
...
class TLSManager(Object):
    """This class configures the certificates within OpenBao."""
    ...
    def _configure_self_signed_certificates(self, _: EventBase) -> None:  # noqa: C901
        ...
        if self.tls_access:
            logger.warning(
                "TLS access relation found, skipping self signed certificate generation."
            )
            return
        ...
        if self.charm.unit.is_leader():
            if not secret_ca_certificate:
                ca_private_key, ca_certificate = generate_openbao_ca_certificate()
        ...
        if not ca_private_key:
            ...
            return
        unit_private_key, unit_certificate = generate_openbao_unit_certificate(
            common_name=self.common_name,
            sans_dns=self.sans_dns,
            sans_ip=self.sans_ip,
        ...
        logger.info(
            "Saved OpenBao generated CA and self signed certificate to %s.",
            self.juju_facade.unit_name,
        )
        self._restart_openbao()
    ...
    def _restart_openbao(self) -> None:
        """Attempt to restart the OpenBao server."""
        try:
            self.workload.restart(self._service_name)
            logger.debug("OpenBao restarted")
        except Exception:
            logger.debug("Couldn't restart OpenBao. Proceeding normally.")
...
def generate_openbao_ca_certificate() -> tuple[str, str]:
    """Generate OpenBao CA certificates valid for 50 years.
...
def generate_openbao_unit_certificate(
    ...
    """Generate OpenBao unit certificates valid for 50 years.
    ...
    """
    openbao_private_key = generate_private_key()
    csr = generate_csr(
        private_key=openbao_private_key,
        common_name=common_name,
        sans_ip=sans_ip,
        sans_dns=sans_dns,
    )
    openbao_certificate = generate_certificate(
        ca=Certificate.from_string(ca_certificate),
        ca_private_key=PrivateKey.from_string(ca_private_key),
        csr=csr,
        validity=timedelta(days=365 * 50),
    )
    return str(openbao_private_key), str(openbao_certificate)


def existing_certificate_is_self_signed(ca_certificate: Certificate) -> bool:
    """Return whether the certificate is a self signed certificate generated by the OpenBao charm."""
    return ca_certificate.common_name == OPENBAO_CA_SUBJECT


class Naming:
    """Computes names for OpenBao features.

    This class is used to compute names for OpenBao features based on the charm's
    ...
    backup_s3_key_prefix: str = "openbao-backup-"
    kv_mount_prefix: str = "charm-"
    kv_secret_prefix: str = "openbao-kv-"
...
class _PKIUtils:
    ...
    def __init__(self, openbao_client: OpenBaoClient, mount_point: str):
        self._openbao_client = openbao_client
        self._mount_point = mount_point

    def get_openbao_service_ca_certificate(self) -> Certificate | None:
        """Get the current intermediate CA certificate from the OpenBao service."""
        try:
            intermediate_ca_cert = self._openbao_client.get_intermediate_ca(
                mount=self._mount_point
            )
            if not intermediate_ca_cert:
                return None
            return Certificate.from_string(intermediate_ca_cert)
        except (OpenBaoClientError, TLSCertificatesError) as e:
    ...
    def make_latest_issuer_default(self):
        ...
        """
        try:
            first_issuer = self._openbao_client.list_pki_issuers(mount=self._mount_point)[0]
        except (OpenBaoClientError, IndexError) as e:
            logger.error("Failed to get the first issuer: %s", e)
            return
        try:
            issuers_config = self._openbao_client.read(path=f"{self._mount_point}/config/issuers")
    ...
    def calculate_certificates_ttl(self, certificate: Certificate) -> int:
        """Calculate the maximum allowed validity of certificates issued by OpenBao.
    ...
    def intermediate_ca_exceeds_role_ttl(
        ...
        OpenBao PKI enforces that issued certificates cannot outlast their signing CA.
...
class AutounsealProviderManager:
    ...
    OpenBao charms. It is opinionated, and aims to make the interface to enabling
    ...
    def __init__(
        ...
        provides: OpenBaoAutounsealProvides,
        ca_cert: str,
        mount_path: str,
    ):
    ...
    def _detect_and_allow_deletion_of_orphaned_keys(self) -> None:
        ...
        The keys are marked as `allow_deletion` in openbao. This allows the user
        to manually delete the keys using the OpenBao CLI if they are sure the
        ...
        if not orphaned_keys:
            return
        logger.warning(
            "Orphaned autounseal keys were detected: %s. If you are sure these are no longer needed, you may manually delete them using the openbao CLI to suppress this message. To delete a key, use the command `openbao delete %s/keys/<key_name>`.",
            orphaned_keys,
            self.mount_path,
        )
    ...
    def create_credentials(self, relation: Relation, openbao_address: str) -> tuple[str, str, str]:
        """Create auto-unseal credentials for the given relation.

        Args:
            relation: The relation to create the credentials for.
            openbao_address: The address where this relation can reach the OpenBao.
    ...
    def get_outstanding_requests(self) -> list[Relation]:
        ...
        - Have credentials in Juju that are stale in OpenBao (e.g. after a
...
class AutounsealRequirerManager:
    ...
    In other words, this manages the feature from the perspective of the OpenBao
    being auto-unsealed.
    """
    ...
    def __init__(
        self,
        charm: CharmBase,
        requires: OpenBaoAutounsealRequires,
    ):
        self._juju_facade = JujuFacade(charm)
        self._requires = requires

    def get_provider_openbao_token(
        self, autounseal_details: AutounsealDetails, ca_cert_path: str
    ) -> str:
        """Retrieve the auto-unseal OpenBao token, or generate a new one if required.
        ...
        Args:
            ...
            ca_cert_path: The path to the CA certificate to validate the provider OpenBao.

        Returns:
            A periodic OpenBao token that can be used for auto-unseal.

        """
        external_openbao = OpenBaoClient(url=autounseal_details.address, ca_cert_path=ca_cert_path)
        try:
        ...
        try:
            token_valid = existing_token and external_openbao.authenticate(Token(existing_token))
        except OpenBaoAuthenticationError:
            token_valid = False
        if not token_valid:
            external_openbao.authenticate(
            ...
            if self._juju_facade.is_leader:
                self._juju_facade.set_app_secret_content(
                    {"token": external_openbao.token},
                    label=self.AUTOUNSEAL_TOKEN_SECRET_LABEL,
                )
        return external_openbao.token


class PKIManager:
    """Encapsulates the business logic for managing PKI certificates in OpenBao from a Charm."""

    SELF_SIGNED_CA_SECRET_LABEL = "openbao-pki-self-signed-ca"
    ...
    def __init__(
        ...
        Args:
            ...
            openbao_client: The OpenBao client object
            ...
            mount_point: The mount point in OpenBao for the PKI backend
            role_name: The role name for the PKI backend
            openbao_pki: The openbao_pki provider relation helper library
    ...
    def _get_pki_intermediate_ca_from_relation(
        ...
        This is the CA certificate that the provider charm has issued to OpenBao.
        """
        ...
        if not provider_certificate:
            logger.debug("No intermediate CA certificate available for OpenBao PKI")
        if not private_key:
            logger.debug("No private key available for OpenBao PKI")
        return provider_certificate, private_key

    def configure(self):
        """Enable the PKI backend and update the PKI role in OpenBao.

        This method retrieves the intermediate certificate from the relation
        and configures the PKI role in OpenBao if they have changed (or haven't
        ...
        if not self._juju_facade.is_leader:
            logger.debug("Only leader unit can handle a openbao-pki certificate requests")
            return
    ...
    def _configure_external_ca(self):
        ...
        if not certificate_from_provider or not private_key:
            return
        openbao_service_ca_certificate = self._pki_utils.get_openbao_service_ca_certificate()
        if (
            openbao_service_ca_certificate
            and openbao_service_ca_certificate == certificate_from_provider.certificate
        ):
            current_ttl = self._openbao_client.get_role_max_ttl(
                role=self._role_name, mount=self._mount_point
            )
            if current_ttl and not self._pki_utils.intermediate_ca_exceeds_role_ttl(
                openbao_service_ca_certificate, current_ttl
            ):
        ...
        if update_ca_certificate:
            self._openbao_client.import_ca_certificate_and_key(
    ...
    def _configure_self_signed_ca(self):
        """Configure the PKI backend using a OpenBao self-signed CA."""
        ca_certificate = self._ensure_self_signed_ca_in_openbao()
        issued_certificates_validity = self._pki_utils.calculate_certificates_ttl(ca_certificate)
        self._create_or_update_pki_role(issued_certificates_validity)
        self.make_latest_pki_issuer_default()

    def _ensure_self_signed_ca_in_openbao(self) -> Certificate:
        """Ensure a self-signed CA exists in both Juju secrets and OpenBao.

        Handles three scenarios:
        1. CA exists in secret and OpenBao - no action needed (idempotent)
        2. CA exists in secret but not OpenBao - re-import (e.g. after Raft restore)
        3. No CA or config changed - generate a new one

        Returns:
            The CA certificate that is now active in OpenBao.
        """
        ...
        if existing_cert and existing_key:
            ...
            # CA in secret matches OpenBao - nothing to do
            openbao_service_ca_certificate = self._pki_utils.get_openbao_service_ca_certificate()
            if openbao_service_ca_certificate and openbao_service_ca_certificate == existing_cert:
                logger.debug("Self-signed CA already configured in the PKI secrets engine")
                return existing_cert

            # CA exists in secret but not in OpenBao (e.g. after Raft snapshot restore)
            self._openbao_client.import_ca_certificate_and_key(
    ...
    def _generate_and_store_self_signed_ca(self) -> Certificate:
        ...
        # generate_root already creates the issuer in OpenBao, no need to import
        self._juju_facade.set_app_secret_content(
    ...
    def _generate_self_signed_ca(self) -> tuple[str, str]:
        """Generate a new self-signed CA using OpenBao.

        Returns:
            tuple: (certificate_pem, private_key_pem)
        """
        attrs = self._certificate_request_attributes
        return self._openbao_client.generate_self_signed_ca(
            mount=self._mount_point,
    ...
    def _create_or_update_pki_role(self, issued_certificates_validity: int) -> None:
        ...
        if not self._openbao_client.role_config_matches_given_config(
        ...
        ) or issued_certificates_validity != self._openbao_client.get_role_max_ttl(
            role=self._role_name, mount=self._mount_point
        ):
            self._openbao_client.create_or_update_pki_charm_role(
    ...
    def sync(self):
        ...
        """
        if not self._juju_facade.is_leader:
            logger.debug("Only leader unit can handle a openbao-pki request")
            return
        if not self._self_signed_ca and not self._juju_facade.relation_exists(
            ...
            return
        outstanding_pki_requests = self._openbao_pki.get_outstanding_certificate_requests()
    ...
    def _get_ca_certificate(self) -> Certificate | None:
        ...
        if self._self_signed_ca:
            ...
            if cert:
                return cert
            openbao_ca = self._pki_utils.get_openbao_service_ca_certificate()
            if openbao_ca:
                return openbao_ca
            return None
    ...
    def _generate_pki_certificate_for_requirer(self, requirer_csr: RequirerCertificateRequest):
        if not self._openbao_client.is_pki_role_created(
            role=self._role_name, mount=self._mount_point
        ):
        ...
        if not ca_certificate:
            logger.debug(
                "No CA certificate available for the PKI backend, cannot sign certificate requests"
            )
            return
        allowed_cert_validity = self._pki_utils.calculate_certificates_ttl(ca_certificate)
        try:
            certificate = self._openbao_client.sign_pki_certificate_signing_request(
                mount=self._mount_point,
                role=self._role_name,
        ...
        self._openbao_pki.set_relation_certificate(
    ...
    def _report_certificate_request_error(
        ...
        self._openbao_pki.set_relation_error(
    ...
    def _allowed_domains_list(self) -> list[str]:
        """Return the allowed domains for the ACME server in OpenBao as a list."""
...
class KVManager:
    ...
    def generate_credentials_for_requirer(
        ...
        Args:
            ...
            openbao_url: The URL of the OpenBao server that the requirer can access
                over this relation.
        """
        if not self._juju_facade.is_leader:
        ...
        self._openbao_client.enable_secrets_engine(SecretsBackend.KV_V2, mount)
    ...
    def _remove_stale_nonce(self, relation: Relation, nonce: str) -> None:
        ...
        """
        credential_nonces = self._openbao_kv.get_credentials(relation).keys()
        if nonce not in set(credential_nonces):
            self._openbao_kv.remove_unit_credentials(relation, nonce=nonce)
    ...
    def _ensure_unit_credentials(
        ...
        """Ensure a unit has credentials to access the openbao-kv mount.
        ...
        Otherwise, the necessary OpenBao policy and approle are created or
        updated as necessary.  A OpenBao secret ID is then generated for the
        ...
        current_credentials = self._openbao_kv.get_credentials(relation)
        credentials_juju_secret_id = current_credentials.get(nonce, "")

        if self._is_openbao_kv_role_configured(
            label=juju_secret_label,
            egress_subnets=egress_subnets,
            role_name=role_name,
            credentials_juju_secret_id=credentials_juju_secret_id,
        ):
            logger.info("OpenBao KV role already configured for the provided egress subnets")
            return credentials_juju_secret_id
        self._openbao_client.create_or_update_policy(policy_name, KV_POLICY.format(mount=mount))
        role_id = self._openbao_client.create_or_update_approle(
            role_name,
            policies=[policy_name],
        ...
        role_secret_id = self._openbao_client.generate_role_secret_id(role_name, egress_subnets)
    ...
    def _is_openbao_kv_role_configured(
        self,
        label: str,
        egress_subnets: list[str],
        role_name: str,
        ...
        """Check if the OpenBao role is already configured for the provided egress subnets.
        ...
        if not role_secret_id:
            return False
        try:
            role_data = self._openbao_client.read_role_secret(role_name, role_secret_id)
        except OpenBaoClientError:
            logger.warning(
                "Failed to read role secret for %s, will re-create credentials", role_name
            )
            return False
...
class BackupManager:
    """Encapsulates the business logic for managing backups in OpenBao from a Charm.

    This class provides the business logic for creating, listing, and restoring
    backups of the OpenBao data.
    """
    ...
    def create_backup(self, openbao_client: OpenBaoClient, skip_verify: bool = False) -> str:
        """Create a backup of the OpenBao data.

        Stores the backup in the S3 bucket provided by the S3 relation.

        Args:
            openbao_client: The OpenBao client to use for creating the snapshot
        ...
        response = openbao_client.create_snapshot()
    ...
    def restore_backup(
        self, openbao_client: OpenBaoClient, backup_key: str, skip_verify: bool = False
    ) -> None:
        """Restore the OpenBao data from the backup using the ``openbao_client`` provided.

        Args:
            openbao_client: The OpenBao client to use for restoring the snapshot
        ...
        try:
            openbao_client.restore_snapshot(snapshot=snapshot)
...
class RaftManager:
    """Encapsulates the business logic for managing the bootstrap of a OpenBao cluster in Raft mode."""
    ...
    def bootstrap(self, node_id: str, address: str) -> None:
        """Bootstrap a OpenBao cluster in Raft mode.

        This method will bootstrap a OpenBao cluster for a single node, by
        identifying itself as the sole node in the cluster. Additional units
        may then be added once the cluster is available.
        """
        if not self._juju_facade.is_leader:
            logger.debug("Only leader unit can bootstrap a OpenBao cluster")
            raise ManagerError("Only the leader unit can bootstrap a OpenBao cluster")
        if not self._juju_facade.planned_units_for_app == 1:
            raise ManagerError("Bootstrapping a OpenBao cluster requires exactly one unit")

        self._workload.stop(self._service_name)
        self._push_peers_json(node_id, address)
        self._workload.restart(self._service_name)

        logger.info("OpenBao cluster bootstrapped in Raft mode")

    def _push_peers_json(self, node_id: str, address: str) -> None:
        """Create the peers.json file for the OpenBao cluster.

        This method will create the peers.json file for the OpenBao cluster based
...
class ACMEManager:
    """Encapsulates the business logic for managing the ACME engine in OpenBao."""
    ...
    def __init__(
        ...
        self._certificate_request_attributes = certificate_request_attributes
        self._role_name = role_name
        self._openbao_address = openbao_address
        self._pki_utils = _PKIUtils(openbao_client, mount_point)
        self._allowed_domains = (
            allowed_domains if allowed_domains else certificate_request_attributes.common_name
        )
    ...
    def _get_acme_intermediate_ca_from_relation(
        ...
        if not provider_certificate:
            logger.debug("No intermediate CA certificate available for OpenBao ACME")
        if not private_key:
            logger.debug("No private key available for OpenBao ACME")
        return provider_certificate, private_key

    def _configure_intermediate_ca_certificate(self) -> None:
        """Configure the intermediate CA certificate for the ACME server in OpenBao."""
        certificate_from_provider, private_key = self._get_acme_intermediate_ca_from_relation()
        if not certificate_from_provider or not private_key:
            raise ManagerError("No intermediate CA certificate available for OpenBao ACME")
    ...
    def _configure_acme_role(self) -> None:
        """Configure the ACME role in OpenBao."""
        ...
        if not self._openbao_client.role_config_matches_given_config(
            ...
            province=self._province,
            locality=self._locality,
        ) or max_ttl != self._openbao_client.get_role_max_ttl(
            role=self._role_name, mount=self._mount_point
        ):
            self._openbao_client.create_or_update_acme_role(
                role=self._role_name,
                max_ttl=f"{max_ttl}s",
                mount=self._mount_point,
                allow_bare_domains=self._allow_bare_domains,
    ...
    def configure(self) -> None:
        """Configure the ACME server in OpenBao."""
        if not self._juju_facade.is_leader:
        ...
        self._openbao_client.allow_acme_headers(
            mount=self._mount_point,
        )
        self._openbao_client.set_urls(
            issuing_certificates=[f"{self._openbao_address}/v1/pki/ca"],
            crl_distribution_points=[f"{self._openbao_address}/v1/pki/crl"],
            mount=self._mount_point,
        )
        self._openbao_client.write(
            path=f"{self._mount_point}/config/cluster",
            data={"path": f"{self._openbao_address}/v1/{self._mount_point}"},
        )
        self._enable_acme()
