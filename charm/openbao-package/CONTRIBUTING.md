...
This package contains common code shared between the `k8s` and `machine` `openbao` charms. Updates to this code must be vendored into both of these charms. To do so, run `make vendor-shared-code` in the `k8s/` and `machine/` directories, or run any of their `tox` commands. Ensure that all changed files and any newly created files are committed to version control.
