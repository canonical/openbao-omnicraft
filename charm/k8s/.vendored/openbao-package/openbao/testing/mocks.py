...
from openbao.openbao_client import OpenBaoClient
...
class OpenBaoCharmFixturesBase:
    """Base class for OpenBaoCharmFixtures in k8s and machine charms."""
    ...
    def mocks(self) -> Iterator[None]:
        ...
        with contextlib.ExitStack() as stack:
            ...
            self.mock_openbao = stack.enter_context(
                patch("charm.OpenBaoClient", autospec=OpenBaoClient)
            ).return_value
            self.mock_openbao.generate_self_signed_ca.return_value = ("cert-pem", "key-pem")
            self.mock_openbao_autounseal_provider_manager = stack.enter_context(
            ...
            self.mock_openbao_autounseal_requirer_manager = stack.enter_context(
            ...
            self.mock_autounseal_provides_set_data = stack.enter_context(
                patch("charm.OpenBaoAutounsealProvides.set_autounseal_data")
            )
