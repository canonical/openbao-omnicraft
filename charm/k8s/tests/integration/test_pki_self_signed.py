...
def deploy(
    ...
    juju.deploy(
        ...
        else OPENBAO_PKI_REQUIRER_APPLICATION_NAME,
        OPENBAO_PKI_REQUIRER_APPLICATION_NAME,
...
def test_given_self_signed_ca_configured_when_integrate_openbao_pki_then_certificate_is_issued(
    ...
    with fast_forward(juju, JUJU_FAST_INTERVAL):
        juju.wait(
            lambda s: (
                jubilant.all_active(s, APPLICATION_NAME, OPENBAO_PKI_REQUIRER_APPLICATION_NAME)
                and len(s.apps[APPLICATION_NAME].units) == NUM_OPENBAO_UNITS
    ...
    task = juju.run(f"{OPENBAO_PKI_REQUIRER_APPLICATION_NAME}/0", "get-certificate", {}, wait=240)
