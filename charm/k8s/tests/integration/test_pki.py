...
@pytest.mark.abort_on_fail
def test_given_openbao_pki_relation_and_unmatching_common_name_when_integrate_then_cert_not_provided(
    juju: jubilant.Juju, deploy: OpenBaoInit
):
    juju.integrate(
        f"{APPLICATION_NAME}:openbao-pki",
    ...
    with fast_forward(juju, JUJU_FAST_INTERVAL):
        juju.wait(
            lambda s: (
                ...
                and jubilant.all_active(s, OPENBAO_PKI_REQUIRER_APPLICATION_NAME)
                and len(s.apps[APPLICATION_NAME].units) == NUM_OPENBAO_UNITS
            ),
            timeout=SHORT_TIMEOUT,
        )
...
@pytest.mark.abort_on_fail
def test_given_openbao_pki_relation_and_matching_common_name_configured_when_integrate_then_cert_is_provided(  # noqa: E501
    juju: jubilant.Juju, deploy: OpenBaoInit
):
    juju.config(
        APPLICATION_NAME,
    ...
    with fast_forward(juju, JUJU_FAST_INTERVAL):
        ...
        wait_for_status_message(
            ...
            app_name=OPENBAO_PKI_REQUIRER_APPLICATION_NAME,
            count=1,
            timeout=SHORT_TIMEOUT,
        )
