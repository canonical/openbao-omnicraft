...
class OpenBao:
    ...
    def wait_for_node_to_be_unsealed(self) -> None:
        ...
        Args:
            endpoint (str): The endpoint of the openbao unit
    ...
    def write(self, path: str, data: dict) -> None:
        ...
        logger.info("Wrote data to OpenBao at path: %s", path)

    def read(self, path: str) -> dict | None:
        """Read data from OpenBao.
        ...
        if isinstance(response, dict):
            ...
            raise Exception(
                "Unexpected response type from OpenBao client. Expected a requests.Response."
            )
        logger.info("Read data from OpenBao at path: %s", path)
        return json.get("data", {})

    def delete(self, path: str) -> None:
        """Delete data from OpenBao.

        Args:
            path (str): The path to delete data from
        """
        self.client.delete(path=path)
        logger.info("Deleted data from OpenBao at path: %s", path)
