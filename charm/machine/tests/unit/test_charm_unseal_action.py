...
class TestCharmUnsealAction(OpenBaoCharmFixtures):
    def _peer_state(self, secrets=None, leader=True):
    ...
    def test_given_secret_id_not_found_when_unseal_then_fails(self):
        ...
        assert "could not be found by the charm" in e.value.message
    ...
    def test_given_key_missing_from_secret_when_unseal_then_fails(self):
        ...
        with pytest.raises(ActionFailed) as e:
            self.ctx.run(
                ...
                self._peer_state(secrets=[secret]),
            )
        assert "Unseal key not found" in e.value.message
    ...
    def test_given_auto_unseal_when_unseal_then_fails(self):
        ...
        assert "auto-unseal" in e.value.message

    def test_given_invalid_key_when_unseal_then_fails_without_logging_key(self):
        self.mock_openbao.configure_mock(
            **{
                "is_api_available.return_value": True,
                "is_initialized.return_value": True,
        ...
        assert e.value.message == "Failed to unseal OpenBao"
        assert "super-secret-key" not in e.value.message
