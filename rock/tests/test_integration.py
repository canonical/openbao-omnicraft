...
"""Integration tests to validate that the OpenBao ROCK works as expected."""
    ...
    while time.time() - initial_time < timeout:
        try:
            if vault.is_api_available():
                logger.info("OpenBao API is available")
                return
        except Exception:
            pass
        time.sleep(1)
    raise TimeoutError("OpenBao is not available after {} seconds".format(timeout))


@pytest.fixture(scope="module")
def run_container():
    """Start a OpenBao container."""
    subprocess.check_call(
        "docker run -d -p 8200:8200 openbao-rock:test",
        shell=True,
    )
