# Recover a OpenBao Cluster When Raft Quorum is Lost

## Prerequisites

1. A OpenBao cluster that has lost quorum

## 1. Scale the Cluster Down to One Node

On the machine charm, this means removing all but one unit. Ideally, keep the unit that is the leader, but otherwise choose any healthy unit that is in the blocked "Waiting for openbao to finish raft leader election" state.

```shell
juju remove-unit openbao/1 openbao/2 openbao/3 openbao/4
```
...
```shell
juju scale-application openbao-k8s 1
```
...
If necessary, follow the instructions on how to unseal OpenBao in the Unseal a sealed unit guide.
...
```shell
juju add-unit openbao -n 4
```

Or, on the Kubernetes charm:

```shell
juju scale-application openbao-k8s 5
```
