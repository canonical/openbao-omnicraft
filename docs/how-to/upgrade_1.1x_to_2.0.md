...
This guide covers the upgrade path from any OpenBao charm 1.1x version (1.15, 1.16, 1.17, 1.18, 1.19) to 2.0. For general upgrade instructions, see the [upgrade guide](upgrade.md).

## Pre-requisites

- OpenBao 1.1x is deployed and in `active/idle` state.
...
- **OpenBao workload version**: The OpenBao binary is upgraded to 2.0.x. While HashiCorp maintains backward compatibility for the storage format, this is a major version jump.
...
- **Configuration key rename (from 1.16 or earlier)**: The `common_name` configuration option was renamed to `pki_ca_common_name` in 1.19. If you are upgrading from 1.16 or earlier and using the `openbao-pki` relation, you must record the current value before upgrading and re-apply it using the new key after the upgrade.
...
Make sure to save your unseal key(s), as the upgrade operation will seal all units. You will need these keys to unseal OpenBao after the upgrade.

### 3. Record current PKI configuration (from 1.16 or earlier only)

If you are upgrading from 1.16 or earlier and using the `openbao-pki` relation, save your current `common_name` value:

```shell
juju config openbao common_name
```
...
```shell
juju refresh openbao --channel=2.0/stable
```

If upgrading from **1.16 or earlier** (base change required):

```shell
juju refresh openbao --channel=2.0/stable --base ubuntu@24.04
```

### 5. Reconfigure PKI (from 1.16 or earlier only)

If you were using the `openbao-pki` relation and upgraded from 1.16 or earlier, set the new configuration key:

```shell
juju config openbao pki_ca_common_name=<your-common-name>
```

### 6. Unseal OpenBao

Unseal all OpenBao units using your saved unseal keys. See the unseal guide for your platform:
